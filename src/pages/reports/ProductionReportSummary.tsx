...
/**
 * Production Report – Summary (with/without revenue) by customer, delivery date or date range.
 * PRD: Indicate final / draft.
 */
const ProductionReportSummary = () => {
  const [params, setParams] = useState<{ startDate?: string; endDate?: string; reportType?: string }>({})
  const formApi = useForm<FieldValues>({
    defaultValues: { startDate: '', endDate: '', reportType: 'summary', includeRevenue: true },
  })

  const { data, isFetching } = useQuery({
    queryKey: ['production-report-summary', params],
    queryFn: async () => {
  ...
  const onSubmit = (values: FieldValues) => {
    setParams({
      ...
      includeRevenue: values.includeRevenue,
    } as JsonObject)
  }

  return (
    <div>
      <ModuleBreadCrumb pageTitle="Production Report – Summary" />
        ...
        <div className="card-body">
          <FormProvider {...formApi}>
            <form onSubmit={formApi.handleSubmit(onSubmit)} className="flex flex-wrap items-end gap-4">
              ...
              <div className="min-w-[140px]">
                <FormField name="includeRevenue" label="Include revenue" type="checkbox" />
              </div>
      ...
      <div className="card mt-4">
        <div className="card-header flex items-center justify-between">
          <h3 className="text-lg font-semibold">Summary by customer / delivery date</h3>
