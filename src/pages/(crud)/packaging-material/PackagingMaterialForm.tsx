...
export const PackagingMaterialForm = ({ isUpdateRecord, isViewRecord, fetchRecord, toggle }: CrudFormProps<PackagingMaterialPayload>) => {
  const API = useModuleApi()
  const { useSubmitHandler } = API.crudApi.crudHandler

  const formApi = useForm<PackagingMaterialPayload>({
    mode: 'all',
    defaultValues: fetchRecord ?? {},
  })

  const { handleSubmit } = formApi
  const { isPending, mutate } = useSubmitHandler<PackagingMaterialPayload>()

  const submitHandler = (data: PackagingMaterialPayload) => {
    ...
    >
      <FormProvider {...formApi}>
        <form onSubmit={handleSubmit(submitHandler)} className="space-y-4">
          <FormField name="name" label="Name" validateRule={{ required: true, name: 'Name' }} placeholder="e.g. Cardboard Box" />
        </form>
      </FormProvider>
    </SideDrawer>
  )
