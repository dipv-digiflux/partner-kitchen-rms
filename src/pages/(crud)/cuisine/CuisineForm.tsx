...
export const CuisineForm = ({ isUpdateRecord, isViewRecord, fetchRecord, toggle }: CrudFormProps<CuisinePayload>) => {
  const API = useModuleApi()
  const { useSubmitHandler } = API.crudApi.crudHandler

  const formApi = useForm<CuisinePayload>({
    mode: 'all',
    defaultValues: fetchRecord ?? {},
  })

  const { handleSubmit } = formApi
  const { isPending, mutate } = useSubmitHandler<CuisinePayload>()

  const submitHandler = (data: CuisinePayload) => {
    ...
    >
      <FormProvider {...formApi}>
        <form onSubmit={handleSubmit(submitHandler)} className="space-y-4">
          <FormField name="name" label="Name" validateRule={{ required: true, name: 'Name' }} placeholder="e.g. Italian" />
        </form>
      </FormProvider>
    </SideDrawer>
  )
